...
#define TRACER_TRACE_MEMBER_FUNC_GEN_BEFORE_SIGNAL											\
	struct BeforeSignal {																	\
		typedef																				\
			::tracer::FoldParameters<														\
				::tracer::AllParamsToRef<													\
					::tracer::PrependParameter<												\
						::tracer::SetResultType<											\
							::tracer::MemberToNormal<Signature>::type,						\
							void															\
						>::type,															\
						bool																\
					>::type																	\
...
#define TRACER_TRACE_MEMBER_FUNC_GEN_AFTER_SIGNAL											\
	struct AfterSignal {																	\
		typedef																				\
			::tracer::FoldParameters<														\
				::tracer::PrependParameter<													\
...
#define TRACER_TRACE_MEMBER_FUNC_CLASS_NAME BOOST_PP_CAT(Tracer, __LINE__)

#define TRACER_TRACE_MEMBER_FUNC(func)														\
	struct TRACER_TRACE_MEMBER_FUNC_CLASS_NAME {											\
		typedef decltype(&func) Signature;													\
		TRACER_TRACE_MEMBER_FUNC_GEN_BEFORE_SIGNAL											\
		TRACER_TRACE_MEMBER_FUNC_GEN_AFTER_SIGNAL											\
	private:																				\
		::tracer::Signal<BeforeSignal::Signature> &before_signal_;							\
		::tracer::Signal<AfterSignal::Signature> &after_signal_;							\
	public:																					\
		TRACER_TRACE_MEMBER_FUNC_CLASS_NAME() :												\
			before_signal_(::tracer::SignalKeeper<BeforeSignal>::signal),					\
			after_signal_(::tracer::SignalKeeper<AfterSignal>::signal) {					\
			::tracer::RealFuncKeeper<TRACER_TRACE_MEMBER_FUNC_CLASS_NAME>::real = &func;	\
			if (NO_ERROR != ::tracer::HookManager::Instance().Install(						\
				reinterpret_cast<void**>(&::tracer::RealFuncKeeper<							\
					TRACER_TRACE_MEMBER_FUNC_CLASS_NAME>::real),							\
				reinterpret_cast<void*&>(::tracer::FakeFuncKeeper<							\
					TRACER_TRACE_MEMBER_FUNC_CLASS_NAME>::fake)))							\
				throw ::std::runtime_error(BOOST_PP_STRINGIZE(								\
						BOOST_PP_CAT(BOOST_PP_CAT(Hook,										\
							TRACER_TRACE_MEMBER_FUNC_CLASS_NAME), fail)));					\
		}																					\
		~TRACER_TRACE_MEMBER_FUNC_CLASS_NAME() {											\
			::tracer::HookManager::Instance().Remove(reinterpret_cast<void**>(				\
				&::tracer::RealFuncKeeper<TRACER_TRACE_MEMBER_FUNC_CLASS_NAME>::real));		\
		}																					\
